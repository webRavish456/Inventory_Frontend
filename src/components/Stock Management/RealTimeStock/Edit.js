...
import { updateRealTimeStock } from "@/lib/stockApi";
...
const EditRealTimeStock = ({ stockData, onClose, onSave }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      setLoading(true);
      const response = await updateRealTimeStock(stockData._id, formData);
      if (response?.success) {
        setError("");
        if (onSave) {
      ...
      setError("Error updating: " + (err.message || "Unknown error"));
      console.error(err);
    } finally {
      setLoading(false);
    }
