...
const EditCapacity = ({ capacity, warehouses = [], handleUpdate, handleClose }) => {
  ...
  useEffect(() => {
    if (capacity) {
      setFormData({
        warehouseName: capacity.warehouseName || '',
        totalZonesInWarehouse: capacity.totalZonesInWarehouse || '',
        totalCapacityUnits: capacity.totalCapacityUnits || '',
        totalCapacityVolume: capacity.totalCapacityVolume || '',
        totalCapacityWeight: capacity.totalCapacityWeight || '',
        availableCapacityVolume: capacity.availableCapacityVolume || '',
        availableCapacityWeight: capacity.availableCapacityWeight || '',
        reservedCapacity: capacity.reservedCapacity || '',
        utilizationPercent: capacity.utilizationPercent || 0,
        throughputCapacity: capacity.throughputCapacity || '',
        status: capacity.status || 'Good',
        lastUpdated: new Date().toISOString().split('T')[0]
      });

      // Set zone data if it exists
      if (capacity.zoneData) {
        setZoneData(capacity.zoneData);
      } else {
        // Generate zone data from totalZonesInWarehouse
        const numZones = parseInt(capacity.totalZonesInWarehouse) || 0;
        const newZoneData = {};
  ...
  const handleSave = () => {
    if (validateForm()) {
      ...
      const warehouseId = wh?.id || wh?.warehouseId || capacity?.warehouseId;
